...
class Employee(db.Model):
    ...
    @property
    def department(self):
        """إرجاع اسم القسم للموظف - للموظفين في المديرية يكون اسم المدرسة هو القسم"""
        if self.is_directorate_employee and self.school:
...
class User(UserMixin, db.Model):
    ...
    can_delete_school = db.Column(db.Boolean, default=False)  # نقل هذا العمود إلى هنا
    ...
    # Report permissions
    # إزالة الأعمدة المكررة من السطور 448-455
    # احذف هذه الأسطر:
    ...
    # احتفظ فقط بالأعمدة الأصلية في السطور 414-431
    can_view_leave_reports = db.Column(db.Boolean, default=False)
    can_view_departure_reports = db.Column(db.Boolean, default=False)
    ...
    def has_permission(self, permission=None):
        ...
        permission_map = {
            ...
            # في دالة has_permission، السطر حوالي 250
            'can_manage_employees': (getattr(self, 'can_edit_employees_data', False) and 
                                   getattr(self, 'can_add_new_employee', False) and 
                                   getattr(self, 'can_delete_employee', False)),
...
# Remove all the permission columns from here - they should only be in the User model
# احذف جميع الأعمدة من السطر 384 إلى 450 لأنها خارج نموذج User
# هذه الأعمدة يجب أن تكون داخل نموذج User فقط
...
class UserLog(db.Model):
    ...
    def __repr__(self):
        return f'<UserLog {self.id}: {self.user_id} - {self.action} - {self.module}>'

class PrincipalCasualLeave(db.Model):
    """نموذج لتسجيل الإجازات العرضية لمدراء المدارس"""
    __tablename__ = 'principal_casual_leaves'
    
    id = db.Column(db.Integer, primary_key=True)
    employee_id = db.Column(db.Integer, db.ForeignKey('employee.id'), nullable=False)  # الموظف (مدير المدرسة)
    school_id = db.Column(db.Integer, db.ForeignKey('school.id'), nullable=False)  # المدرسة
    school_letter_number = db.Column(db.String(100), nullable=False)  # رقم كتاب المدرسة
    school_letter_date = db.Column(db.Date, nullable=False)  # تاريخ كتاب المدرسة
    leave_reason = db.Column(db.Text, nullable=False)  # سبب الإجازة
    start_date = db.Column(db.Date, nullable=False)  # تاريخ بداية الإجازة
    end_date = db.Column(db.Date, nullable=False)  # تاريخ نهاية الإجازة
    days_count = db.Column(db.Integer, nullable=False)  # عدد أيام الإجازة
    ...
    # العلاقات
    employee = db.relationship('Employee', backref=db.backref('principal_casual_leaves', lazy=True))
    school = db.relationship('School', backref=db.backref('principal_casual_leaves', lazy=True))
    
    def __repr__(self):
        return f'<PrincipalCasualLeave {self.id}: {self.employee.name} - {self.start_date} to {self.end_date}>'

class Penalty(db.Model):
    """نموذج لتسجيل العقوبات"""
    __tablename__ = 'penalties'
    
    id = db.Column(db.Integer, primary_key=True)
    employee_id = db.Column(db.Integer, db.ForeignKey('employee.id'), nullable=False)  # الموظف
    penalty_type = db.Column(db.String(200), nullable=False)  # نوع العقوبة
    school_id = db.Column(db.Integer, db.ForeignKey('school.id'), nullable=False)  # المدرسة/القسم
    letter_number = db.Column(db.String(100), nullable=False)  # رقم الكتاب
    letter_date = db.Column(db.Date, nullable=False)  # تاريخ الكتاب
    notes = db.Column(db.Text, nullable=True)  # ملاحظات إضافية
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # العلاقات
    employee = db.relationship('Employee', backref=db.backref('penalties', lazy=True))
    school = db.relationship('School', backref=db.backref('penalties', lazy=True))
    
    def __repr__(self):
        return f'<Penalty {self.id}: {self.employee.name} - {self.penalty_type}>'

class InvestigationCommittee(db.Model):
    """نموذج لتشكيل لجان التحقيق"""
    __tablename__ = 'investigation_committees'
    
    id = db.Column(db.Integer, primary_key=True)
    school_id = db.Column(db.Integer, db.ForeignKey('school.id'), nullable=False)  # المدرسة أو القسم
    committee_head_id = db.Column(db.Integer, db.ForeignKey('employee.id'), nullable=False)  # رئيس اللجنة
    first_member_id = db.Column(db.Integer, db.ForeignKey('employee.id'), nullable=False)  # العضو الأول
    second_member_id = db.Column(db.Integer, db.ForeignKey('employee.id'), nullable=False)  # العضو الثاني
    complaint_content = db.Column(db.Text, nullable=False)  # مضمون الشكوى
    committee_date = db.Column(db.Date, nullable=False)  # تاريخ تشكيل اللجنة
    letter_number = db.Column(db.String(100), nullable=True)  # رقم الكتاب
    letter_date = db.Column(db.Date, nullable=True)  # تاريخ الكتاب
    status = db.Column(db.String(50), default='نشطة')  # حالة اللجنة (نشطة، مكتملة، ملغاة)
    notes = db.Column(db.Text, nullable=True)  # ملاحظات إضافية
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # العلاقات
    school = db.relationship('School', backref=db.backref('investigation_committees', lazy=True))
    committee_head = db.relationship('Employee', foreign_keys=[committee_head_id], backref=db.backref('headed_committees', lazy=True))
    first_member = db.relationship('Employee', foreign_keys=[first_member_id], backref=db.backref('first_member_committees', lazy=True))
    second_member = db.relationship('Employee', foreign_keys=[second_member_id], backref=db.backref('second_member_committees', lazy=True))
    
    def __repr__(self):
        return f'<InvestigationCommittee {self.id}: {self.school.name} - {self.committee_date}>'